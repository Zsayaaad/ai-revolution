...
const Navbar = () => {
  ...
  return (
    <nav className="bg-white/80 backdrop-blur-md shadow-sm fixed w-full z-50">
      <div className="app-container flex justify-between items-center h-16">
        ...
        <div className="hidden md:flex items-center space-x-8">
          {links.map((link, index) => (
            <a key={index} href={link.href} className="nav-item">
              {link.name}
            </a>
          ))}
      ...
      {/* Mobile Menu */}
      {mobileMenuOpen && (
        <div className="md:hidden bg-white shadow-lg rounded-lg space-y-1 px-2 pt-2 ">
          {links.map((link, index) => (
            <a key={index} href={link.href} className="mobile-nav-item">
              {link.name}
            </a>
          ))}
